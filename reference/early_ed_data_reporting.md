...
Similarly, there's the possibility for conflicting information to be overwritten on updates to parent records if the same unique ID is used. In order to reduce the changes of that, MDE requires that a parent ID be prefixed with a district/organization number and dash (using the [Ed-Fi identifier conventions](../sis_test_plan/sis_test_plan_b_cert_testing.md#minnesota-district-and-school-ids)), such as this:
